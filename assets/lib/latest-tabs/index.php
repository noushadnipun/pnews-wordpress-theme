...
function kento_latest_tabs_scripts($hook) {
        /* Register our script. */
		wp_enqueue_style( 'KENTO_LATEST_TABS_STYLE', get_bloginfo('template_url').'/assets/lib/latest-tabs/css/style.css' );		 
        wp_enqueue_script( 'jquery');
		wp_enqueue_script( 'kento-highlight', get_bloginfo('template_url').'/assets/lib/latest-tabs/js/kento-highlight.js', array('jquery'));
...
class Kento_Latest_Tabs_Plugin extends WP_Widget {
	...
	function Kento_Latest_Tabs_Plugin() {
		...
		$this->WP_Widget( 'kento-latest-tabs-widget', __('Home Latest Tabs Widget', 'kento_latest_tabs_plugin'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$hi_view_post = new WP_Query( apply_filters( 'widget_posts_args', array( 'posts_per_page' => 3, 'more' => $more = 0, 'meta_key' => 'views', 'no_found_rows' => true, 'post_status' => 'publish',  'orderby' =>'meta_value_num', 'order' => 'DESC', 'ignore_sticky_posts' => true ) ) ); 
		
		$hi_com_post = new WP_Query( apply_filters( 'widget_posts_args', array( 'posts_per_page' => 3, 'no_found_rows' => true, 'post_status' => 'publish',  'orderby' =>'comment_count', 'ignore_sticky_posts' => true ) ) ); 
		
		$recent = new WP_Query( apply_filters( 'widget_posts_args', array('posts_per_page' => 3, 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true ) ) ); 
	if ($hi_com_post->have_posts()) :

		echo $before_widget; ?>
	
        
        <div id="kento-highlight-widget" style="margin-bottom: 0px;">			<!-- Start of tabbed widget -->
	...
	<?php } ?>
        ...
        <div id="tab1" class="tabs-wrap" style="display:block;">
								<ul> 
                                <?php while( $hi_view_post->have_posts() ) : $hi_view_post->the_post(); ?>
									...
									<?php endwhile; ?>
                                    <?php endif; ?>
								<li><a style="text-align: center; color: #c00 !important;" href="<?php echo get_bloginfo('home'); ?>/most-viewed"><b>আরও</b></a></li>
								</ul>
							</div>				<!-- end of tab1 -->
								...
								?>
									...
									<!--
                                        ...
                                        <a id="dot1" href="<?php echo get_permalink($comment->comment_post_ID); ?>#comment-<?php echo $comment->comment_ID; ?>"><?php $str = $comment->comment_content; 
				                         ...
				                         if ( $len_count >=51){		
                                         echo '<b>'. get_the_title( $comment->comment_post_ID ) . '</b>'; 				               
				                         //echo  '<b>'.($comment->comment_author . ':</b>' .substr($str,0,50).'...' )  ;
				                         }
              				
			                            else {
					                   echo '<b>'. get_the_title( $comment->comment_post_ID ) . '</b>' /* . '<b>'.($comment->comment_author . ':</b> ' .$str ) */ ; } ?></a>
                                   -->
											
									<li>
									<a style="font-weight: bold !important;" id="dot1" href="<?php the_permalink() ?>" title="<?php echo esc_attr( get_the_title() ? get_the_title() : get_the_ID() ); ?>"><?php if ( get_the_title() ) the_title(); else the_ID(); ?></a>
									...
									endwhile;
									?>
									<li><a style="text-align: center; color: #c00 !important;" href="<?php echo get_bloginfo('home'); ?>/most-popular"><b>আরও</b></a></li>
								</ul>
							</div>			<!-- end of tab3 -->
        ...
        <div id="tab2" class="tabs-wrap" style="display:none;">
								<ul> <?php if ($recent->have_posts()) : ?>
                                    ...
                                    <div class="clear"></div>
									<?php endwhile; ?>
									<li><a style="text-align: center; color: #c00 !important;" href="<?php echo get_bloginfo('home'); ?>/archive"><b>আরও</b></a></li>
								</ul>							
							</div>		<!-- end of tab2 -->
...
function kento_latest_tabs_admin_page() {
	add_menu_page(__('Home Latest Tabs','kentothemes'), __('Home Latest Tabs','kentothemes'), 'manage_options', 'latesttabssettings', 'kh_settings_page');
}
