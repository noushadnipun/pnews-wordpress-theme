...
// Set default css styles ---------------->
	function ba_admin_styles()
	{
		...
		if(is_admin() ||  ba_is_login_page() )	{
			wp_enqueue_style( get_ba_options('slug'), get_bloginfo('template_url') . '/assets/lib/admin-dashboard/assets/default-css/style.css',false,get_ba_options('version'),false );
		}
		// main adminbar css styles ---------------->
		if(is_user_logged_in()){
			 wp_enqueue_style(get_ba_options('slug').'_admin_bar', get_bloginfo('template_url'). '/assets/lib/admin-dashboard/assets/default-css/adminbar.css',false, get_ba_options('version'),false );
		}
		// BA page styles ---------------->
		if( is_user_logged_in() && is_admin() ){ 
			if(isset($_GET['tab']) && trim($_GET['tab'])=='blue_admin_login_page'){
				wp_enqueue_media();
				wp_enqueue_script( 'ba-color-picker',  get_template_directory(). ('/assets/js/ba-color-picker.js' ),'', false, true ); 
			}
    	}
	}
